...
#data = b[,-c(9:14,16,18:21)]


#cria uma coluna para armazenar o tipo de vinho e o preço
...
#REBALANCEAMENTO DO MODELO (há muito mais Tinto do que branco e espumante, por exemplo) 
#table(data$tipo_vinho)
#table(data$Faixa_Preco)


#separa uma nova amostra de treino
TrainingSetB = TrainingSet

#downsample a amostra para balancear os tipos de vinho
set.seed(400)

TrainingSetB = downSample(x=TrainingSet, as.factor(TrainingSet$tipo_vinho), list = FALSE)
table(TrainingSetB$tipo_vinho)
table(TrainingSetB$Faixa_Preco)
table(TrainingSetB$Nac_do_vinho)


#remove a nova coluna criada para fazer o downsample (ela atrapalharia o modelo)
TrainingSetB = TrainingSetB[,-10]

x = TrainingSetB
x$tipo_vinho = as.factor(x$tipo_vinho)
...
#O MODELO ESTÁ OK (>90%) PARA SUGERIR UM TIPO DE VINHO (TINTO, ETC)
...
for(i in 1:nrow(recom3)) {
  j = (recom3[i,9])
  if (j == "Tinto"){
    recom3[i,10] = sample(curtidos[(curtidos$tipo_vinho=="Tinto"),]$name, size = 1, replace = FALSE)  
  }
  if (j == "Rose"){
    recom3[i,10] = sample(curtidos[(curtidos$tipo_vinho=="Rose"),]$name, size = 1, replace = FALSE)  
  }
  if (j == "Espumante"){
    recom3[i,10] = sample(curtidos[(curtidos$tipo_vinho=="Espumante"),]$name, size = 1, replace = FALSE)  
  }
